...
$sql = "SELECT incentivado.invnum,dateini,datefin,codpro 
	...
	where incentivado.estado = '1' and incentivadodet.estado = '1'
    ...
    while ($row = mysqli_fetch_array($result)) 
    {
        ...
        if (mysqli_num_rows($resultX)) {
            while ($rowX = mysqli_fetch_array($resultX)) {
                $invnum = $rowX['invnum'];
                mysqli_query($conexion,"UPDATE detalle_venta set incentivo = '$invnumIncentivo' where invnum = '$invnum' and codpro = $codpro");
            }
        }
    }
}
