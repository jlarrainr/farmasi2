...
if ($local == 'all')
{
$sql="SELECT usecod FROM venta where invfec between '$date1' and '$date2' and val_habil = '0' group by usecod order by nrovent";
}
else
{
$sql="SELECT usecod FROM venta where invfec between '$date1' and '$date2' and val_habil = '0' and sucursal = '$local' group by usecod order by nrovent";
}
...
<body>
 <table width="954" border="0">
    <tr>
        <td><b><u>REPORTE DE CURVA ABC</u></b>
          ...
          <tr>
			...
			<select name="local" id="local">
              ...
              <?php } ?>
                        ...
                        <td width="50"><div align="right">BUSCAR POR</div></td>
                        <td width="200">
                            <select name="TipBusk" id="TipBusk">
                                <option <?php if ($TipBusk == 1){?>selected<?php }?> value="1">Todos los Productos</option>
                                <option <?php if ($TipBusk == 2){?>selected<?php }?> value="2">Lo que se ha vendido y se agot�</option>
			...
			<td width="24">
			  ...
			  {
			  ?>
              <a href="curva1.php?val=<?php echo $val?>&date1=<?php echo $date1?>&date2=<?php echo $date2?>&local=<?php echo $local?>&inicio=<?php echo $inicio?>&registros=<?php echo $registros?>&pagina=<?php echo $pagina-1?>"><img src="../../images/play1.gif" width="16" height="16" border="0"/> </a>
              <?php }
			  ?></td>
			<td width="24"><?php if(($pagina + 1)<=$total_paginas) 
			  {
			  ?>
              <a href="curva1.php?val=<?php echo $val?>&date1=<?php echo $date1?>&date2=<?php echo $date2?>&local=<?php echo $local?>&inicio=<?php echo $inicio?>&registros=<?php echo $registros?>&pagina=<?php echo $pagina+1?>"> <img src="../../images/play.gif" width="16" height="16" border="0"/> </a>
              <?php }
			  ?></td>
          </tr>
        </table>
