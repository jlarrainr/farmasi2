...
if(isset($_REQUEST['getCountriesByLetters']) && isset($_REQUEST['letters'])){
	...
	{
		$res = mysqli_query($conexion,"select codcli,descli,dnicli from cliente where ((dnicli = '".$letters."') || (ruccli = '".$letters."')||( descli like '%".$letters."%')) limit $limit") or die(mysqli_error());
		while($inf = mysqli_fetch_array($res)){
