...
$sql = "SELECT canpro,fraccion FROM cotizacion_det where codet = '$codtemp'";
...
if ($cantidades <> 0) {
            ...
            if (($zzcodloc <> 1) && ($precios_por_local == 1)) {
                ...
                if (mysqli_num_rows($result_precio)) {
                    while ($row_precio = mysqli_fetch_array($result_precio)) {
                        $costpr                 = $row_precio['costpr'];
                        $cantventaparabonificar = $row_precio['cantventaparabonificar'];
                        $codprobonif            = $row_precio['codprobonif'];
                        $cantbonificable        = $row_precio['cantbonificable'];
                    }
                }
            }
    ...
    if (($cantidades <> "") || ($cantidades <> 0)) {
        ...
        if ($cant_prod <> 0) {
            ...
            // mysqli_query($conexion, "UPDATE cotizacion_det set canpro = '$permitido',fraccion = '$strFactor',prisal = '$t2',pripro = '$permitidon',costpr='$costpr' where codet = '$codtemp'");

            $sql1 = "UPDATE cotizacion_det set canpro = '$permitido',fraccion = '$strFactor',prisal = '$t2',pripro = '$permitidon',costpr='$costpr' where codet = '$codtemp'";
                    ...
                    if (mysqli_num_rows($resultB)) {
                        while ($rowB = mysqli_fetch_array($resultB)) {
                            ...
                            if ($cantbonificable <= $cant_locB) {
                                ...
                                // mysqli_query($conexion, "INSERT INTO cotizacion_det (invnum,invfec,cuscod,usecod,codpro,canpro,fraccion,factor,prisal,pripro,codmar,cospro,costpr,bonif) values ('$venta','$date','$cuscod','$usuario','$codprobonif','$cantbonificable','T','$factorB','0','0','$codmarB','0','0','1')");
                            
                            
                            
                                $sql2 = "INSERT INTO cotizacion_det (invnum,invfec,cuscod,usecod,codpro,canpro,fraccion,factor,prisal,pripro,codmar,cospro,costpr,bonif) values ('$venta','$date','$cuscod','$usuario','$codprobonif','$cantbonificable','T','$factorB','0','0','$codmarB','0','0','1')";
