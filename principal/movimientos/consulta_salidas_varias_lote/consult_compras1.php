...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head><meta http-equiv="Content-Type" content="text/html; charset=utf-8">
            ...
            $sql = "SELECT count(invnum) FROM movmae where tipmov = '2' and tipdoc = '6' and proceso = '0' and sucursal = '$codigo_local'";
...
//////////////////////////////////
            if (($numero == "") && ($pag == "")) {
                $sql = "SELECT numdoc FROM movmae where tipmov = '2' and tipdoc = '6' and proceso = '0' and sucursal = '$codigo_local' order by numdoc desc limit 1";
//echo "1";
            } else {
                if (($numero == "") && ($pag <> "")) {
                    $sql = "SELECT numdoc FROM movmae where tipmov = '2' and tipdoc = '6' and proceso = '0' and sucursal = '$codigo_local' order by numdoc asc $limit";
                    //echo "2";
                } else {
                    $sql = "SELECT numdoc FROM movmae where numdoc = '$numero' and tipmov = '2' and tipdoc = '6' and proceso = '0' and sucursal = '$codigo_local' order by numdoc desc";
                    //echo "3";
                }
            }
            $result = mysqli_query($conexion, $sql);
            ...
            <table width="100%" border="0">
                <tr>
                    <td width="958">
                        <table width="100%" border="0">
                            <tr>
                                <td width="129" class="LETRA">NUMERO DE MOVIMIENTO </td>
                                <td width="497">
                        ...
                        <?php
                        if (($numero == "") && ($pag == "")) {
                            $sql = "SELECT * FROM movmae where tipmov = '2' and tipdoc = '6' and proceso = '0' and sucursal = '$codigo_local'  order by numdoc desc limit 1";
                        } else {
                            if (($numero == "") && ($pag <> "")) {
                                $sql = "SELECT * FROM movmae where tipmov = '2' and tipdoc = '6' and proceso = '0' and sucursal = '$codigo_local'  order by numdoc $limit";
                            } else {
                                $sql = "SELECT * FROM movmae where numdoc = '$numero' and tipmov = '2' and tipdoc = '6' and proceso = '0' and sucursal = '$codigo_local'  order by numdoc desc";
                            }
                        }
                        $result = mysqli_query($conexion, $sql);
                        if (mysqli_num_rows($result)) {
...
//						error_log("Stock actual: ".$stopro." ".$stock_loc);
                                            if (($cantidad_compra > $stock_loc) || ($cantidad_compra > $stopro)) {
                                                $est_habil_stock = 0;
                                                $msg_habil_stock = "No existe suficiente stock del producto para anular la compra";
                                            } else if ($numlote != '') {
                                                ...
                                                if (mysqli_num_rows($resultLot)) {
                                                    while ($rowLot = mysqli_fetch_array($resultLot)) {
                                                        ...
                                                        if ($cantidad_compra > $stockLote) {
                                                            $est_habil_stock = 0;
                                                            $msg_habil_stock = "No existe suficiente stock en el lote " . $numlote . " para anular la compra";
                                                        }
                                                    }
                                                }
                                            }
                            ...
                            <table width="100%" border="0" >
                                <tr>
                                    ...
                                    <td width="67" class="LETRA">USUARIO:  </td>
                                     ...
                                     <td width="20" class="LETRA">FECHA:  </td>
                                    <td width="10">
                                        <input class="LETRA2" name="fecha" type="readonly" id="fecha" size="10" disabled="disabled" value="<?php echo  fecha($fecha)?>" /></td>
                                  ...
                                  <!--  <td width="39" class="LETRA">N DOC </td> 
                                    ...
                                    <!-- <td width="65" class="LETRA">MONEDA</td>
                                    <td width="142">
                                                  ...
                                                  <!--  <input name="nuevo" type="button" id="nuevo" value="Nuevo" class="nuevo" disabled="disabled"/>
                                                    ...
                                                    <?php if ($val_habil == 1) { ?>
                                                        <?php if ($est_habil_stock == 1) {
                                                            ?>
                                                            <input name="habilitar_salida" type="button" id="habilitar_salida" value="Habilitar" class="eliminar" <?php if ($find <> 1) { ?>disabled="disabled" <?php } ?> onclick="habilitar_salida1()"/>
                                                            <?php
                                                        } else {
                                                            ?>
                                                            <input name="habilitar_salida" type="button" id="habilitar_salida" value="Habilitar" class="eliminar" <?php
...
//														error_log($stringAlert);
                                                            ...
                                                            ?> />
                                                               ...
                                                               ?>												
                                                           <?php } else { ?>
                                                        <input name="anular_salida" type="button" id="anular_salida" value="Anular" class="eliminar" <?php if ($find <> 1) { ?>disabled="disabled" <?php } ?> onclick="anular_salida1()"/>												
                                                    <?php } ?>
