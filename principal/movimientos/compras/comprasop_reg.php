...
$date1  		= fecha1($_POST['date1']);			///cantidad ingresada
$date2  		= fecha1($_POST['date2']);			///precio sin descuentos y sin igv
...
if (mysqli_num_rows($result)) {
	while ($row = mysqli_fetch_array($result)) {
		...
		$sqlx = "SELECT tcosto,tmargene,tprevta,tpreuni,codpro FROM producto where codpro = '$codpro'";
		$resultx = mysqli_query($conexion, $sqlx);
		if (mysqli_num_rows($resultx)) {
			while ($rowx = mysqli_fetch_array($resultx)) {
				$codpro          = $rowx["codpro"];			//codigo
				...
				if (($zzcodloc <> 1) && ($precios_por_local == 1)) {
					...
					$sql_precio = "SELECT $tcosto_p,$tmargene_p,$tprevta_p,$tpreuni_p FROM precios_por_local where codpro = '$codpro'";
		...
		if (($tmargene <> 0) && ($tprevta <> 0) && ($tpreuni <> 0)) {
			...
			if ($precios_por_local == 1) {
				...
				if (($zzcodloc == 1)) {
					...
					mysqli_query($conexion, "UPDATE precios_por_local set $margene_p = '$tmargene',$tmargene_p = '0',$prevta_p= '$tprevta',$tprevta_p= '0',$preuni_p = '$tpreuni',$tpreuni_p = '0',$tcosto_p = '0' where codpro = '$codpro'");
				}
			} else {
		...
		if ($nohay <> 1)	/////PREGUNTO SI EL PRODUCTO ES BONIFICABLE O SI ESTE NO TIENE VINCULO CON BONIFICACION
		{
			...
			$sql1 = "SELECT factor,stopro,preuni,margene,igv,$tabla,pcostouni,codpro FROM producto where codpro = '$codpro'";
			...
			*/
			...
			if ($precios_por_local == 1) {
				...
				if (($zzcodloc == 1)) {
					...
					mysqli_query($conexion, "UPDATE producto set stopro = '$stopro',$tabla = '$cant_local',fecord = '0000-00-00',modifpcosto = '0' where codpro = '$codpro'");
					mysqli_query($conexion, "UPDATE precios_por_local set $prelis_p = '$referencial',$costpr_p = '$costpr',$costre_p = '$pripro',$pcostouni_p = '$pcostouni',$ultpcostouni_p = '$ultpcostouni' where codpro = '$codpro'");
				}
			} else {
...
if ($nohay <> 1) {
	...
	mysqli_query($conexion, "UPDATE movmae set usecod  = '$usuario',fecdoc = '$date1',numero_documento = '$n1', numero_documento1 = '$n2',plazo = '$plazo',moneda = 'S',invtot = '$mont5',destot = '$mont2', valven = '$mont3',fecven = '$date2',monto = '$sum33',costo = '$mont1',igv = '$mont4', estado = '1', proceso = '0',sucursal = '$codloc' where invnum = '$cod'");
	mysqli_query($conexion, "UPDATE ordmae set pendiente = '0', confirmado = '0' where invnum = '$ncompra'");
	header("Location: comprasop_reg1.php");
} else {
