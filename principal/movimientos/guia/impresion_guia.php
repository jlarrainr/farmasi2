...
$vencimnnn = "";
$numlote = "";
$sql1_movlote = "SELECT numlote,vencim FROM movlote where codpro = '$codigo'  and codloc= '$sucursal'  and stock <> 0 and date_format(str_to_date(concat('01/',vencim),'%d/%m/%Y'),'%Y-%m-%d') >= NOW() ORDER BY date_format(str_to_date(concat('01/',vencim),'%d/%m/%Y'),'%Y-%m-%d') limit 1";
...
if (mysqli_num_rows($result1_movlote)) {
 ...
 $sqlUsuX = "SELECT logo,seriegui FROM xcompa where codloc = '$sucursal'";
    ...
    $sqlDespatch = "SELECT lineid, deliveredQuantity, unitCode, nameProduct FROM sd_despatch_line WHERE despatch_id= '$despatch_id'";
    $resultDespatch = mysqli_query($conexion, $sqlDespatch);
    
    //consultar Despatch remitente y destinatario Adrian - impresion_guia.php
    
    $sqlDespatch2 = "SELECT despatch_id,deliverycustomeraccountid,deliveryname,partyidentification,partyname,originAddressStreetName,deliveryStreetName,carrierPartyName,issueDate,issueTime,startDate,unitCodeWeightMeasure,grossWeightMeasure,totalUnitQuantity,handlingCode,information,note,transportModeCode,licensePlateId FROM sd_despatch WHERE despatch_id = '$despatch_id' LIMIT 1;";
  ...
  <meta charset="UTF-8">
    ...
    <title>Document</title>
     ...
     <script>
            ...
            //self.close();
            <?php
            echo "parent.opener.location='guia1.php?val=0';";
