...
$sql = "SELECT numdoc,invfec,tipmov,tipdoc,sucursal,transferenciaVenta FROM movmae where invnum = '$cod'";
...
if (mysqli_num_rows($result)) {
	while ($row = mysqli_fetch_array($result)) {
		...
		mysqli_query($conexion, "UPDATE venta set  val_habil=1 where invnum = '$transferenciaVenta'");


		/////////////////////////////////////////////////////////////
		mysqli_query($conexion, "INSERT INTO kardex (nrodoc,codpro,fecha,tipmov,tipdoc,qtypro,fraccion,factor,invnum,usecod,sactual,sucursal, eliminado,numlote) values ('$numdoc','$codpro','$fechaActual','$tipmov','$tipdoc','$qtypro','$qtyprf','$factor','$cod','$usuario','$sactual','$sucursal','1','$numlote')");
	}
}
