...
if (mysqli_num_rows($result)) {
    while ($row = mysqli_fetch_array($result)) {
        ...
        $sqlProd = "UPDATE producto set stopro = '$stopro', $tabla = '$cant_local' where codpro = '$codpro'";
//		error_log($sqlProd);
        mysqli_query($conexion, $sqlProd);

        $sqlLote = "UPDATE movlote set stock = stock + $cant_unid where codloc='$sucursal' and numlote='$numlote' and codpro = '$codpro'";
//		error_log($sqlLote);
        ...
        $sqlKardex = "INSERT INTO kardex (codpro, nrodoc, tipmov, tipdoc, fecha, qtypro,fraccion,factor, invnum,usecod,sactual,sucursal, eliminado) VALUES('$codpro', '$numdoc', '2', '1', '$fechaActual','$qtypro','$qtyprf','$factor','$cod','$usuario','$sactual','$sucursal', '3')";
//		error_log($sqlKardex);
