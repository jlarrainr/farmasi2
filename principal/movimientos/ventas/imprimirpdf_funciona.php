...
$sql="SELECT desemp,direccionemp,rucemp,telefonoemp FROM datagen";
$result = mysqli_query($conexion,$sql);
if (mysqli_num_rows($result)){
while ($row = mysqli_fetch_array($result)){
		$desemp       = $row['desemp'];
...
$sql="SELECT fdisp,flinpag,fini,fcanti,fcodpro,fmarca,fpreuni,fmonto,fdescuento,fnom,fref,fuente,contit,anchocod,anchonom,anchomarca,anchoreferencial,anchodescuento,anchocantidad,anchoprecio,anchosubtotal FROM xcompa inner join xcompadetalle on xcompa.codloc = xcompadetalle.codloc where xcompa.codloc = '$sucursal' and tipdoc = '$rd'";
$result = mysqli_query($conexion,$sql);
if (mysqli_num_rows($result)){
while ($row = mysqli_fetch_array($result)){
		$fdisp         = $row['fdisp'];
