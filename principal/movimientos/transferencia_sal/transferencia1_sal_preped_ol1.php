...
//}
$alertMensaje = 'No se encontro prepedido';
if ($prepedido > 0) {
    ...
    $sql = "SELECT PRE.codloc,DP.idprod,DP.solicitado,DP.factor,DP.fraccion,DP.numpagina, P.costpr,P.preuni,P.costre FROM prepedido as PRE INNER JOIN detalle_prepedido AS DP ON DP.idprepedido=PRE.idpreped INNER JOIN producto AS P ON DP.idprod=P.codpro INNER JOIN titultabladet AS M ON M.codtab=P.codmar WHERE DP.numpagina= '$prepedido' and DP.solicitado<> '0' and DP.solicitado<> '' order by M.destab,P.desprod ";
    error_log($sql);
