...
if (isset($_REQUEST['getCountriesByLetters']) && isset($_REQUEST['letters'])) {
    ...
    if ($t == 0) {
        ...
        if (strpos($letters, $caracter) !== false) {
            $res = mysqli_query($conexion, "select codpro,desprod,codmar,$columna,factor from producto where eliminado='0' and ( codpro = '" . $letters . "' or codbar like '" . $letters . "' ) order by codpro limit $limit") or die(mysqli_error());
        } else {
