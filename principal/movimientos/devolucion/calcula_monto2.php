<?php
$sql = "SELECT count(*) FROM detalle_nota where invnum = '$credito'";
...
$sql = "SELECT count(*) FROM detalle_nota where invnum = '$credito' and canpro = '0'";
$result = mysqli_query($conexion, $sql);
if (mysqli_num_rows($result)) {
    while ($row = mysqli_fetch_array($result)) {
        $count1        = $row[0];
...
$sqlVenta    = "SELECT sucursal FROM nota where invnum = '$credito'";
...
$valor_vent1 = 0;
$monto_total = 0;
$sql1 = "SELECT codpro,canpro,fraccion,factor,prisal,pripro,costpr,codmar,bonif FROM detalle_nota where invnum = '$credito'";
$result1 = mysqli_query($conexion, $sql1);
if (mysqli_num_rows($result1)) {
    while ($row1 = mysqli_fetch_array($result1)) {
        $codpro    = $row1['codpro'];
