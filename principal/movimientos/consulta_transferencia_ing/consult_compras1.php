...
<head>
    ...
    $sql = "SELECT count(invnum) FROM movmae where tipmov = '1' and tipdoc = '2' and proceso = '0' and sucursal = '$codigo_local'";
    ...
    //////////////////////////////////
    if (($numero == "") && ($pag == "")) {
        $sql = "SELECT numdoc FROM movmae where tipmov = '1' and tipdoc = '2' and proceso = '0' and sucursal = '$codigo_local' order by numdoc desc limit 1";
        //echo "1";
    } else {
        if (($numero == "") && ($pag <> "")) {
            $sql = "SELECT numdoc FROM movmae where tipmov = '1' and tipdoc = '2' and proceso = '0' and sucursal = '$codigo_local' order by numdoc asc $limit";
            //echo "2";
        } else {
            $sql = "SELECT numdoc FROM movmae where numdoc = '$numero' and tipmov = '1' and tipdoc = '2' and proceso = '0' and sucursal = '$codigo_local' order by numdoc desc";
            //echo "3";
        }
    }
    $result = mysqli_query($conexion, $sql);
        ...
        <table width="100%" border="0">
            <tr>
                <td width="958">
                    ...
                    <?php
                    if (($numero == "") && ($pag == "")) {
                        $sql = "SELECT * FROM movmae where tipmov = '1' and tipdoc = '2' and proceso = '0' and sucursal = '$codigo_local' order by numdoc desc limit 1";
                        $dis = 0;
                    } else {
                        if (($numero == "") && ($pag <> "")) {
                            $sql = "SELECT * FROM movmae where tipmov = '1' and tipdoc = '2' and proceso = '0'  and sucursal = '$codigo_local' order by numdoc asc $limit";
                            $dis = 0;
                        } else {
                            $sql = "SELECT * FROM movmae where numdoc = '$numero' and tipmov = '1' and tipdoc = '2' and proceso = '0' and sucursal = '$codigo_local'  order by numdoc desc";
                            $dis = 1;
                        }
                    }
                    $result = mysqli_query($conexion, $sql);
                    if (mysqli_num_rows($result)) {
                        ...
                        $sql = "SELECT val_habil,estado FROM movmae where invnum = '$invnumrecib'";
                        $result = mysqli_query($conexion, $sql);
                        if (mysqli_num_rows($result)) {
                            while ($row = mysqli_fetch_array($result)) {
                                $val_habilmoante = $row['val_habil'];
                                $estadoante = $row['estado'];
                            }
                        }
                        ...
                        if (mysqli_num_rows($result1)) {
                            while ($row1 = mysqli_fetch_array($result1)) {
                                ...
                                $msg_habil_stock = "";
                                $msg_habil_anterio = "";
                                $sqlSto = "SELECT stopro, $col_stock as stock_loc FROM producto where codpro = '$codpro'";
                                //		error_log($sqlSto);
                        ...
                        $msg_habil_anterio = "DOCUMENTO FUE ANULADO OPERACION IMPOSIBLE ";
                    ?>
                        <table width="100%" border="0">
                            <tr>
                                <td width="67">FECHA DOC </td>
                                <td width="143">
                                    <input name="fecha1" type="text" id="fecha1" size="15" value="<?php echo fecha($fecdoc) ?>" disabled="disabled" />
                                </td>
                                ...
                                <td width="290">
                                    <div align="right" class="text_combo_select"><strong>LOCAL:</strong> <?php echo $nombre_local ?> </div>
                                </td>
                            </tr>
                            <tr>
                                <td>FECHA DIG </td>
                                <td>
                                    <input name="fecha2" type="text" id="fecha2" onfocus="blur()" value="<?php echo fecha($fecha) ?>" size="15" disabled="disabled" />
                                </td>
                                <td>PLAZO</td>
                                <td>
                                    <input name="plazo" type="text" id="plazo" onKeyPress="return acceptNum(event)" size="5" maxlength="3" value="<?php echo $plazo ?>" disabled="disabled" />
                                </td>
                                <td>FECHA VCTO </td>
                                ...
                                <td>
                                    ...
                                    <div align="right"><strong>
                                            ...
                                            if ($val_habil == 1) {
                                            ?>
                                                <span class="login" style="font-size:20px;">ANULADO</span>
                                            <?php
                                            } else {
                                            ?>
                                                <span class="login" style="font-size:20px;">ACTIVO</span>
                                            <?php }
                                            ?>
                                        </strong></div>
                                </td>
                        ...
                        <table width="100%" border="0">
                            <tr>
                                <td width="955">
                                    <iframe src="consult_compras2.php?invnum=<?php echo $invnum ?>&upd=<?php echo $upd ?>&numero=<?php echo $numero ?>" name="iFrame2" width="100%" height="400" scrolling="Automatic" frameborder="0" id="iFrame2" allowtransparency="0"> </iframe>
                                </td>
                            </tr>
                        </table>
                    <?php
                            ...
                            <tr>
                                ...
                                <td width="580"><label>
                                        <div align="right">
                                            ...
                                            <?php if ($val_habil == 1) { ?>
                                                <!-- <input name="del" type="button" id="del" value="Activar" class="eliminar" <?php if (($codgrup <> 2) and ($val_habilmoante <> 0) and ($estadoante <> 0)) { ?>disabled="disabled" <?php } ?> <?php if ($val_habilmoante == 0) { ?> onclick="habilitaring()" <?php
                                                                                                                                                                                                                                                                                                            } else {
                                                                                                                                                                                                                                                                                                                $stringAlert22 = "onclick=\"alert('$msg_habil_anterio');\"";
                                                                                                                                                                                                                                                                                                                echo $stringAlert22;
                                                                                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                                                                                                ?> /> -->
                                            <?php } else { ?>
                                                ...
                                                ?>
                                                    <input title="<?php echo $col_stock ?>" name="del1" type="button" id="del1" value="Anular" class="eliminar" <?php if ($codgrup <> 2) { ?>disabled="disabled" <?php } ?> onclick="eliminaring()" />
                                                <?php
                                                } else {
                                                ?>
