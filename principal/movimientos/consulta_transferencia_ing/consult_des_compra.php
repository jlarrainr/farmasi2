...
$sql = "SELECT numdoc,invfec,tipmov,tipdoc,sucursal,invnumrecib FROM movmae where invnum = '$cod'";
...
if (mysqli_num_rows($result)) {
	while ($row = mysqli_fetch_array($result)) {
		...
		echo $sactual."<br>";
		echo $stopro2."<br>";
		echo $stopro2 ."=".$stopro."-".$cant_unid."<br>";
		echo $cant_unid."<br>";
		echo $cant_local."<br>";
		echo $tabla."<br>";
		echo $invnumrecib."<br>";*/
		mysqli_query($conexion, "UPDATE producto set stopro = '$stopro2', $tabla = '$cant_local' where codpro = '$codpro'");
		mysqli_query($conexion, "UPDATE movlote set stock ='0',eliminado=1 where idlote = '$numlote'");
		/////////////////////////////////////////////////////////////
		mysqli_query($conexion, "INSERT INTO kardex (nrodoc,codpro,fecha,tipmov,tipdoc,qtypro,fraccion,factor,invnum,usecod,sactual,sucursal, eliminado,numlote) values ('$numdoc','$codpro','$fechaActual','1','2','$qtypro','$qtyprf','$factor','$cod','$usuario','$sactual','$sucursal','1','$numlote')");
	}
}
mysqli_query($conexion, "UPDATE movmae set val_habil = '1' ,estado='0' where invnum = '$cod'");
mysqli_query($conexion, "UPDATE movmae set val_habil = '0' ,estado='0' where invnum = '$invnumrecib'");
header("Location: cons_transferencia_ing.php");
