...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        $sql = "SELECT count(invnum) FROM movmae where tipmov = '1' and tipdoc = '5' and proceso = '0' and sucursal = '$codigo_local' ";
...
//////////////////////////////////
        if (($numero == "") && ($pag == "")) {
            $sql = "SELECT numdoc FROM movmae where tipmov = '1' and tipdoc = '5' and proceso = '0' and sucursal = '$codigo_local'   order by numdoc desc limit 1";
//echo "1". "<br>";;
        } else {
            if (($numero <> "") && ($pag <> "")) {
                $sql = "SELECT numdoc FROM movmae where tipmov = '1' and tipdoc = '5' and proceso = '0' and sucursal = '$codigo_local'   order by numdoc asc $limit";
                //   echo "2". "<br>";;
            } else {
                $sql = "SELECT numdoc FROM movmae where numdoc = '$numero' and tipmov = '1' and tipdoc = '5' and proceso = '0' and sucursal = '$codigo_local'   order by numdoc desc";
                // echo "3". "<br>";;
            }
        }
        $result = mysqli_query($conexion, $sql);
            ...
            <table width="100%" border="0">
                <tr>
                    <td width="958">
                        ...
                        <?php
                        if (($numero == "") && ($pag == "")) {
                            $sql = "SELECT * FROM movmae where tipmov = '1' and tipdoc = '5' and proceso = '0'  and sucursal = '$codigo_local'  order by numdoc desc limit 1";
                            $dis = 0;
                        } else {
                            if (($numero <> "") && ($pag <> "")) {
                                $sql = "SELECT * FROM movmae where tipmov = '1' and tipdoc = '5' and proceso = '0' and sucursal = '$codigo_local'   order by numdoc asc $limit";
                                $dis = 0;
                            } else {
                                $sql = "SELECT * FROM movmae where numdoc = '$numero' and tipmov = '1' and tipdoc = '5' and proceso = '0' and sucursal = '$codigo_local'   order by numdoc desc";
                                $dis = 1;
                            }
                        }
                        $result = mysqli_query($conexion, $sql);
                        if (mysqli_num_rows($result)) {
                            ...
                            if (mysqli_num_rows($result1)) {
                                while ($row1 = mysqli_fetch_array($result1)) {
                                    ...
                                    $col_stock = "s" . sprintf('%03d', $codigo_local - 1);
                                    $est_habil_stock = 1;
                                    $msg_habil_stock = "";
...
//							error_log("Stock actual: ".$stopro." ".$stock_loc);
                                            if (($cantidad_compra > $stock_loc) || ($cantidad_compra > $stopro)) {
                                                $est_habil_stock = 0;
                                                $msg_habil_stock = "No existe suficiente stock del producto para anular el ingreso";
                                            } else if ($numlote != '') {
                                                ...
                                                if (mysqli_num_rows($resultLot)) {
                                                    while ($rowLot = mysqli_fetch_array($resultLot)) {
                                                        ...
                                                        if ($cantidad_compra > $stockLote) {
                                                            $est_habil_stock = 0;
                                                            $msg_habil_stock = "No existe suficiente stock en el lote " . $numlote . " para anular el ingreso";
                                                        }
                                                    }
                                                }
                                            }
                        ...
                        if (mysqli_num_rows($result1)) {
                            ...
                            <table width="100%" border="0">
                                <tr>
                                     ...
                                     <td width="67" class="LETRA">REFERENCIA:  </td>
                                    <td width="143">
                                        <input class="LETRA2" name="refere" type="readonly" id="refere" size="45" disabled="disabled" value="<?php echo $refere ?>" /></td>
                                        
                                    <td width="67" class="LETRA">USUARIO:  </td>
                                     <td width="314">   <input class="LETRA2" name="usecod" type="readonly" id="usecod" size="45" disabled="disabled" value="<?php echo $user_venta ?>" /></td>
                            ...
                            <table width="100%" border="0">
                                <tr>
                                    <td width="955">
                                        <iframe src="consult_compras2.php?invnum=<?php echo $invnum ?>&upd=<?php echo $upd ?>&numero=<?php echo $numero ?>" name="iFrame2" width="100%" height="350" scrolling="Automatic" frameborder="0" id="iFrame2" allowtransparency="0"> </iframe></td>
                                </tr>
                            </table>
                            <?php
                        } else {
                            ...
                            <table width="962" border="0">
                                <tr>
                                    <td width="955">
                                        <iframe src="consult_compras3.php" name="iFrame2" width="100%" height="350" scrolling="Automatic" frameborder="0" id="iFrame2" allowtransparency="0"> </iframe></td>
                                </tr>
                            </table>
                        <?php }
                        ?>
                                    ...
                                    <tr>
                                        ...
                                        <td width="580"><label>
                                                <div align="right">
                                                    ...
                                                    <input name="nuevo" type="hidden" id="nuevo" value="Nuevo" class="nuevo" disabled="disabled"/>
                                                    <input name="modif" type="hidden" id="modif" value="Modificar" class="modificar" disabled="disabled" onclick="modificar()"/>

                                                    <?php if ($val_habil == 1) { ?>
                                                        <input name="habilitar_ingreso" type="button" id="habilitar_ingreso" value="Activar" class="eliminar" <?php if ($find <> 1) { ?>disabled="disabled" <?php } ?> onclick="habilitar_ingreso1()"/>
                                                    <?php } else { ?>
                                                        <?php if ($est_habil_stock == 1) {
                                                            ?>
                                                            <input name="anular_ingreso" type="button" id="anular_ingreso" value="Anular" class="eliminar" <?php if ($find <> 1) { ?>disabled="disabled" <?php } ?> onclick="anular_ingreso1()"/>
                                                            <?php
                                                        } else {
                                                            ?>
                                                            <input name="anular_ingreso" type="button" id="anular_ingreso" value="Anular" class="eliminar" <?php
