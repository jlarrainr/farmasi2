...
<head>
 ...
 <link href="../../../css/calendar/calendar.css" rel="stylesheet" type="text/css">
  ...
  <script>
    ...
    function validar() {
      ...
      f.submit();
          ...
          alert("no hay proveedor a buscar, porque no tiene ningun proveedor con deuda.")
      }
    }
  ...
  $sql = "SELECT codpro FROM proveedor where codpro ='$proveedor'   ";
  $result = mysqli_query($conexion, $sql);
  while ($row = mysqli_fetch_array($result)) {
...
<body>
  ...
  <form name="form1" id="form2" onClick="highlight(event)" onKeyUp="highlight(event)">
    <table width="100%" border="0">
      <tr>
        ...
        <td width="140">
          <div align="right" class="LETRA">PROVEEDORES </div>
        </td>
        <td width="300">
          ...
          <select style='width:250px;' id="proveedor" name="proveedor">
            <!--<option value="1" >SELECCIONE UN PROVEEDOR </option>-->
            <?php
            $sql = "SELECT P.codpro,P.despro FROM proveedor as P INNER JOIN movmae as M on M.cuscod=P.`codpro` WHERE M.plazo>0 and M.monto_deuda>0 GROUP by P.codpro order by P.despro ";
            $result = mysqli_query($conexion, $sql);
            while ($row = mysqli_fetch_array($result)) {
              $s_prov = $row["codpro"];
            ?>
              <option value="<?php echo $row["codpro"]; ?>" <?php if ($proveedor == $s_prov) { ?> selected="selected" <?php } ?>><?php echo substr($row["despro"], 0, 55); ?></option>
            <?php } ?>
          </select>
  ...
  ?>
    <form name="form2" id="form2" onClick="highlight(event)" onKeyUp="highlight(event)">
      ...
      //echo '$proveedor = '.$proveedor."<br>";
       ...
       $sql1 = "SELECT cuscod,SUM(monto_deuda) as monto_deuda FROM movmae where cuscod = '$proveedor' AND tipmov='1' AND tipdoc='1' and val_habil ='0' and plazo > 0";
      $result1 = mysqli_query($conexion, $sql1);
      if (mysqli_num_rows($result1)) {
        while ($row1 = mysqli_fetch_array($result1)) {
          $nombreProveedor    = $row1['cuscod'];
      ...
      <table width="100%" border="0">
        ...
        <tr>
          <td>
            ...
            <table width="100%" border="1" class="tabla2" align="center" cellpadding="0" cellspacing="0">
              <thead id="Topicos_Cabecera_Datos">
                <tr bgcolor="#83b9e7">
                  ...
                  <th width="366"><strong>PROVEEDOR</strong></th>
                  <th width="104">
              ...
              <tr>
                ...
                <td width="366"><?php echo $nombreProveedor; ?></td>
                <td width="104">
            ...
            <?php
            $sql = "SELECT invnum,invfec,forpag,numdoc,numero_documento,numero_documento1,plazo,fecven,monto_deuda,proceso,fecha_pago_reprogramacion FROM movmae where cuscod = '$proveedor' AND tipmov='1' AND tipdoc='1' and monto_deuda <> 0 and val_habil ='0' and plazo > 0 order by fecven asc";
            ...
            ?>
              <table width="100%" border="0"  align="center" cellpadding="0" cellspacing="0" scrolling="Automatic"  id="customers">
                <tr bgcolor="#83b9e7">
                  ...
                  <th><div  align="center"> FECHA DE COMPRA</div> </th>
               ...
               <tr>
                    ...
                    <td align="center">
                      <a href="ver_movimiento1.php?invnum=<?php echo $invnumVenta ?>" target="_blank" onclick="window.open(this.href,this.target,'width=1000,height=300,top=180,bottom=200,left=100,right=100,toolbar=no,location=no,status=no,menubar=no');return false;"><strong>
                         <?php echo $nrofactura;?> 
                        </strong>
                        </a>
                    </td>
                    ...
                    <td align="center"><input name="seleccionDeuda" type="radio" id="seleccionDeuda" value="<?php echo $invnumSelect; ?>" <?php if ($seleccionDeuda == $invnumSelect) { ?> checked <?php } ?> <?php if ($i==0) { ?> disabled="disabled" <?php } ?>   onchange="cambia()" />  </td>
            ...
            ?>
              ...
              <?php  
              $monto_deuda22=0;
              $sql1 = "SELECT invnum,monto_deuda,fecven,fecha_pago_reprogramacion FROM movmae where invnum = '$seleccionDeuda' AND tipmov='1' AND tipdoc='1' and val_habil ='0' and plazo > 0 ";
              $result1 = mysqli_query($conexion, $sql1);
              if (mysqli_num_rows($result1)) {
                while ($row1 = mysqli_fetch_array($result1)) {
                  $invnum           = $row1['invnum'];
                  $monto_deudare    = $row1['monto_deuda'];
                  $fecha_pago       = $row1['fecven'];
                  $fechaplazo       = $row1['fecha_pago_reprogramacion'];
                  $textoCuota       = "1/1";
                }
              ...
              ?>
                 ...
                 <fieldset style="width:98%;">
                    <legend style="color:brown;font-size:16px;"> <strong>REALIZAR PAGO</strong></legend>
                ...
                <tr>
                    <td>
                         <div align="right">
                            ...
                            <input name="id" type="hidden" id="id" value="<?php echo $invnum ?>" />
                            <input name="proveedor" type="hidden" id="proveedor" value="<?php echo $proveedor ?>" />
            ...
            <?php } ?>
               ...
               <fieldset style="width:98%;">
                    <legend style="color:brown;font-size:16px;"> <strong>DETALLE DE PAGO </strong></legend>
                    
            
                  <center>
                    <iframe src="por_pagar3.php?proveedor=<?php echo $proveedor ?>" name="iFrame1" width="100%" height="500" scrolling="Automatic" frameborder="0" id="iFrame1" allowtransparency="0">
                    </iframe>
  ...
  ?>
    ...
    <div align="center">
      <h3 class='Estilo2'>
        SELECCIONE UN PROVEEDOR A BUSCAR
      </h3>
    </div>
  <?php // } //CIERRO EL VALID
  } //CIERRO EL VAL
