...
<body bgcolor="#f7f7f7">
 ...
 //echo '$cliente ====== '.$cliente;
  ...
  $sql1 = "SELECT id, fechaplazo, tipoDocumento,id_cuota, formaPago, montoPago, usuario,hora, codigoCliente, fechapago,banco, nrobanco, saldo_deuda,textoCuota,montoTotal,fecha_pago_cuota FROM venta_cuotas_detalle where codigoCliente = '$cliente' order by id desc";
  ...
  $sql1 = "SELECT VCD.id, VCD.usuario,VCD.id_cuota FROM venta_cuotas_detalle as VCD INNER JOIN venta_cuotas as VC on VC.id=VCD.id_cuota where VCD.codigoCliente = '$cliente' GROUP by VC.venta_id order by id desc ";
  $result1 = mysqli_query($conexion, $sql1);
  if (mysqli_num_rows($result1)) {
  ?>
    ...
    <table width="100%" align="center" border="0" id="customers">
      ...
      while ($row1 = mysqli_fetch_array($result1)) {
       ...
       $id_cuota                 = $row1['id_cuota'];
        ...
        $sql = "SELECT venta_id FROM venta_cuotas where id = '$id_cuota'";
        $result = mysqli_query($conexion, $sql);
        if (mysqli_num_rows($result)) {
          while ($row = mysqli_fetch_array($result)) {
      ...
      ?>
        <tbody>
          <tr>
            ...
            <td> <a onclick="popup(<?php echo $venta_id ?>)"><strong>
                         <?php echo $nrofactura;?> 
                        </strong>
                        </a></td>
