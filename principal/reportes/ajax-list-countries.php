...
if (isset($_REQUEST['getCountriesByLetters']) && isset($_REQUEST['letters'])) {
    ...
    if ($limit == 0) {
    	...
    	$busquedaProducto = "0";
$sql11 = "SELECT busquedaProducto FROM datagen";
$result11 = mysqli_query($conexion, $sql11);
if (mysqli_num_rows($result11)) {
	while ($row11 = mysqli_fetch_array($result11)) {
	
		$busquedaProducto    = $row11['busquedaProducto'];
	}
}
...
$healthy=array("ñ","Ñ");
    ...
    if ($t == 0) {
        ...
        if (strpos($limpiacadena, $caracter) !== false) {
            $res = mysqli_query($conexion, "select codpro,desprod,codmar,$columna from producto where codpro  like '" . $limpiacadena . "'  order by desprod limit $limit") or die(mysqli_error());
        } else {
    
        	if ($busquedaProducto == 0) {
            $res = mysqli_query($conexion, "select codpro,desprod,codmar,$columna from producto where desprod like '" . $limpiacadena . "%' order by desprod limit $limit") or die(mysqli_error());
		            	                     } else {
            $res = mysqli_query($conexion, "select codpro,desprod,codmar,$columna from producto where desprod like '%" . $limpiacadena . "%' order by desprod limit $limit") or die(mysqli_error());
		            	                     }
        }
