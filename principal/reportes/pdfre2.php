...
//$pdf->Image('logo.jpeg',7,10,0,0,'jpeg');
 //$pdf->Cell(57, 14, '', 0, 1, 'C');
...
if($numeroCuota >0){ 
 ...
 $pdf->SetFont('Helvetica', 'B', 7);
    $pdf->Cell(18, 10, '# CUOTA', 0, 0, 'L');
    $pdf->Cell(18, 10, 'F EMISION', 0, 0, 'L');
     $pdf->Cell(18, 10, 'F VENCIMIENTO', 0, 0, 'L');
        $pdf->Ln(8);
    $pdf->Cell(60, 0, '', 'T');
    $pdf->Ln(2);
for ($iCuota = 1; $iCuota <= $numeroCuota; $iCuota++) {
            $nuevosDias = $ventaDiasCuotas * $iCuota;
            //$date1 =  $invfec;
        ...
        $pdf->SetFont('Helvetica', '', 7);
        $pdf->Cell(10, 5, $iCuota, 0, 0, 'R');
        
        $pdf->Cell(23, 5, $invfec, 0, 0, 'R');
        $pdf->Cell(18, 5, $fechaPago, 0, 0, 'R');
        $pdf->Ln(3);    
}
...
if (mysqli_num_rows($resultDet)) {
    ...
    while ($row = mysqli_fetch_array($resultDet)) {
        ...
        $height = (ceil(($pdf->GetStringWidth($producto) / $width)) * $line_height);
        ...
        $pdf->MultiCell(30, $height, $producto, 0, 'L');
...
$pdf->Cell(15, 10, number_format($invtot, 2, '.', ''), 0, 0, 'R');
$pdf->Ln(3);
...
if ($tipdoc == 4) {
    ...
    $pdf->Cell(15, 10, number_format(0.00, 2, '.', ''), 0, 0, 'R');
    $pdf->Ln(3);
