                ...
                <table width="100%" border="0" align="center" id="customers">
                    ...
                    if ($tip == 1) {
                        if ($local == 'all') {
                            ...
                            $sql = "SELECT sum(invtot) FROM venta where invfec between '$date1' and '$date2' and val_habil = '0' and sucursal = '$local'  and estado = '0'";
                        }
                        $result = mysqli_query($conexion, $sql);
                        if (mysqli_num_rows($result)) {
                            while ($row = mysqli_fetch_array($result)) {
                        ...
                        if ($ord == 1) {
                            ...
                            if ($local == 'all') {
                                ...
                                $sql = "SELECT sum(DV.pripro),DV.codpro FROM detalle_venta DV inner join venta V on DV.invnum = V.invnum where DV.invfec between '$date1' and '$date2' and V.val_habil = '0' and V.estado = '0' group by DV.codpro order by sum(DV.pripro) asc";
                            } else {

                                $sql = "SELECT sum(DV.pripro),DV.codpro FROM detalle_venta DV inner join venta V on DV.invnum = V.invnum where DV.invfec between '$date1' and '$date2' and V.val_habil = '0' and V.sucursal = '$local' and V.estado = '0' group by DV.codpro order by sum(DV.pripro) asc";
                            }
                        } else {
                            ///todos los productos vendidos 
                            if ($local == 'all') {

                                $sql = "SELECT sum(DV.pripro),DV.codpro FROM detalle_venta DV inner join venta V on DV.invnum = V.invnum where DV.invfec between '$date1' and '$date2' and V.val_habil = '0' and V.estado = '0' group by DV.codpro order by sum(DV.pripro) desc";
                            } else {

                                $sql = "SELECT sum(DV.pripro),DV.codpro FROM detalle_venta DV inner join venta V on DV.invnum = V.invnum where DV.invfec between '$date1' and '$date2' and V.val_habil = '0' and V.sucursal = '$local' and V.estado = '0' group by DV.codpro order by sum(DV.pripro) desc";
                            }
                        }
                        ...
                        if (mysqli_num_rows($result)) {
                            while ($row = mysqli_fetch_array($result)) {
                                ...
                                if ($local == 'all') {
                                    ...
                                    $sql1 = "SELECT V.invnum,DV.canpro,DV.fraccion,DV.factor,DV.prisal,DV.costpr FROM detalle_venta DV inner join venta V on DV.invnum = V.invnum inner join producto P on P.codpro = DV.codpro where DV.invfec between '$date1' and '$date2' and DV.codpro = '$codpro' and V.val_habil = '0' and V.estado = '0'";
                                } else {

                                    $sql1 = "SELECT V.invnum,DV.canpro,DV.fraccion,DV.factor,DV.prisal,DV.costpr FROM detalle_venta DV inner join venta V on DV.invnum = V.invnum inner join producto P on P.codpro = DV.codpro where DV.invfec between '$date1' and '$date2' and DV.codpro = '$codpro' and V.sucursal = '$local' and V.val_habil = '0' and V.estado = '0'";
                                }
                                ...
                                if ($Tabla <> "0") {
                                    $sql1 = "SELECT desprod,codmar,$Tabla,factor,s000,costre,utlcos,blister,codpro,codcatp  FROM producto where codpro = '$codpro' and eliminado='0'";
                                } else {
                                    $sql1 = "SELECT desprod,codmar,stopro,factor,s000,costre,utlcos,blister,codpro,codcatp  FROM producto where codpro = '$codpro' and eliminado='0'";
                                }
                               ...
                               if (mysqli_num_rows($result3)) {
                                ...
                                if (($factor > 1) && ($blister > 0)) {
                                    $redondeo = ($redondeo / $blister) * $blister;
                    ...
                    ?>
                                    <tr height="25" <?php if ($datDSDe2) { ?> bgcolor="#ff0000" <?php } else { ?>onmouseover="this.style.backgroundColor = '#FFFF99';this.style.cursor = 'hand';" onmouseout="this.style.backgroundColor = '#ffffff';" <?php } ?>>
                                        ...
                                        <td width="169" title="<?php echo $factor ?>"><?php echo $destab1; ?></td>
                                        <td width="77">
